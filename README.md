...
Step 0 now uses available-model discovery by default: list candidates from the configured endpoint, probe for multimodal support, and prefer the best match. When the listing misses, it also probes well-known multimodal models as a fallback. If credentials are not available or discovery fails, fall back to the parent model and report UNKNOWN.
