...
After Step 0 detection, apply these rules for subagent spawns:



**Step 0 detection subagent:** Always use parent model (no override). The detection script discovers which model supports multimodal.

**Text/code workers:** Inherit parent model (no override).

**Visual tasks** (images, UI, frontend, screenshots, diagrams, PDFs, charts, rendered output, beautification): Use the multimodal model discovered by Step 0 **when available** (MULTIMODAL). Otherwise inherit parent.

**Screenshot/visual verification subagents:** Use the multimodal model discovered by Step 0 (when available).

**How to override:** Use the model name returned by Step 0 detection (e.g., model="mimo-v2.5"). When Step 0 = UNKNOWN, inherit parent.
...
**This workflow applies ONLY to the main planning agent handling the user's original request.** Spawned subagents (workers, verifiers, diagnostics) MUST NOT run this workflow. Subagents should execute their assigned task directly and return results.



For the MAIN agent, for EVERY user message, follow these steps:



1. **Step 0**: Spawn diagnostic subagent to detect multimodal capabilities (use parent model, no override)
